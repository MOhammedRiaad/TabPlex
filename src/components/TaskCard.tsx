          ...
          <div className="checklist-items">
            {checklist.map(item => (
              <div key={item.id} className="checklist-item-edit">
                ...
                />
                <span>{item.text}</span>
                <button onClick={() => removeChecklistItem(item.id)} className="delete-item-btn">×</button>
              </div>
            ))}
          </div>
    ...
    <div className="task-card">
      ...
      {linkedTabs.length > 0 && (
        <div className="task-links">
          {linkedTabs.map(tab => (
            <a key={tab!.id} href={tab!.url} target="_blank" rel="noopener noreferrer" className="task-link-badge">
              🔗 {tab!.title}
            </a>
          ))}
        </div>
        ...
        <div className="task-checklist">
          ...
          <div className="checklist-preview">
            {task.checklist.map(item => (
              <div key={item.id} className="checklist-item-view" onClick={() => toggleChecklistItem(item.id)}>
